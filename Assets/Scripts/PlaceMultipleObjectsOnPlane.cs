...
public class PlaceMultipleObjectsOnPlane : PressInputBase
{
    ...
    private void Update()
    {
        if ( max >= 2 )
        {
            GetComponent<PlaceMultipleObjectsOnPlane>().enabled = false;
    ...
    protected override void OnPress(Vector3 position)
    {
        ...
        if (aRRaycastManager.Raycast(position, hits, TrackableType.PlaneWithinPolygon)&& cont)
        {
            cont = false;
